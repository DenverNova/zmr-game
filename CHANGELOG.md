...
1. **Spawn** (start of round) — Pick a burst of 1–15 zombies. Each zombie in the burst is independently picked using weighted type selection (60% shambler, 10% each special), so a single wave can contain a **mix of different types**. Before traps are unlocked, spends **all** resources freely. After traps are unlocked, spends only excess **above** the reserve. If a class is blocked by the spawner, at its per-type limit, or unaffordable, its weight is dynamically redistributed equally among the remaining valid types. The AI picks spawners nearest to its **focused target** (the survivor it’s currently picking on) and dynamically updates the chosen spawner mid-burst as that target moves. When the burst completes or resources can't cover another zombie, the AI moves on.
...
**Camera:** The AI ZM camera uses **exponential smoothing** to glide naturally between positions, like a real player panning with WASD and mouse. It positions itself behind and above the focused survivor, smoothly transitioning when switching targets. Teleports to the first target on round start. When the view mode is set to "Within View Only", only spawners, traps, and barrels visible from the camera position are accessible.
