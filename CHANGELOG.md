...
1. **Spawn** (start of round) — Pick a burst of 1–15 zombies. For each zombie in the burst, the AI rolls a weighted class (60% shambler, 10% each special) based on the nearest spawner's allowed types. If a spawner blocks certain types, their weight is **redistributed equally** among the remaining valid types (e.g. a banshee-only spawner gives banshees 100%; blocking hulk+banshee redistributes their 20% equally among the other 3 types). After rolling, the AI **saves up** resources until it can afford that class, then spawns it and rolls the next one. Before traps are unlocked, spends **all** resources freely. After traps are unlocked, holds back the reserve. The AI rechecks the best spawner near its **focused target** every tick — if the survivor moves closer to a different spawner, it dynamically switches spawners and re-rolls the class based on the new spawner's allowed types.
