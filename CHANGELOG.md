...
**Trap opportunism:** The AI continuously monitors trap locations independently of its spawn plan. When survivors walk near a trap, it fires immediately regardless of what the current spawn plan is doing. Trap priority increases when survivors remain in range.

The AI globally tracks all zombie spawners and traps on the map, activates them as needed, and rallies idle zombies toward the nearest survivors. Plan timing, aggression, spawn rate, and trap range are all configurable.

**Hidden spawn:** The AI occasionally places surprise zombies behind survivors using the hidden spawn system (same line-of-sight rules as human ZMs). It picks a random zombie class and searches for valid ground positions near a random survivor. The frequency scales with aggression.
...
| `zm_sv_ai_zm_spawn_range` | `2048` | Legacy range setting (plans now use all spawners globally) |
...
A dedicated **ZMR Settings** tab is available in the Create Server dialog, exposing all server ConVars listed above in a clean UI with dropdowns, sliders, and text fields. No console required for server setup.
...
Font definitions added for 1440p and 4K resolutions so the in-game UI scales correctly on high-DPI displays.
