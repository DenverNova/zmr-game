...
**Phase cycle:** The AI uses a cycle of **Spawn → Hidden Spawn → Reserve (if needed) → Spawn → ...** starting with zombies from the very first tick:

1. **Spawn** — Pick a burst of 1–15 zombies. For each zombie in the burst, the AI rolls a weighted class (60% shambler, 10% each special) based on the nearest spawner's allowed types. If a spawner blocks certain types, their weight is **redistributed equally** among the remaining valid types (e.g. a banshee-only spawner gives banshees 100%; blocking hulk+banshee redistributes their 20% equally among the other 3 types). After rolling, the AI **saves up** until it has enough resources to cover both the zombie's cost **and** the trap reserve (most expensive trap on the map). The moment it can afford `reserve + cost`, it spawns that zombie immediately and rolls the next one — no waiting to batch them all at once. If a trap fires mid-spawn and drops resources below the reserve, the AI does **not** stop to re-reserve. It continues spawning with whatever resources remain (just needs the raw zombie cost). The AI rechecks the best spawner near its **focused target** every tick — if the survivor moves closer to a different spawner, it dynamically switches spawners and re-rolls the class. Server-configured per-type limits (e.g. max 5 hulks) are respected: capped types are excluded from the weighted roll and their weight is redistributed to the remaining types. After all zombies in the burst are spawned → Hidden Spawn.
2. **Hidden Spawn** — Attempt 1–3 surprise spawns near random survivors. Uses the same save-up logic as Spawn (`reserve + cost`). Each spawn is placed **immediately** when affordable — no waiting to accumulate for all at once. Keeps retrying different positions for up to **15 seconds** before giving up. If Hidden Spawn All Classes is enabled, uses weighted selection across all non-limit-capped types; otherwise shamblers only. After done: if a trap was fired at any point during Spawn or Hidden Spawn → Reserve. Otherwise → Spawn.
3. **Reserve** — Only entered when a trap was fired during the Spawn or Hidden Spawn phases and the reserve needs replenishing. The AI saves resources **without spawning anything** until it can cover the most expensive active trap on the map. Once met, traps are unlocked and the AI immediately cycles back to Spawn. If there are no traps on the map, this phase is skipped entirely.

**Traps and explosive barrels:** Once the first reserve target is met, traps and barrels fire **opportunistically during Spawn and Hidden Spawn phases** whenever a survivor enters the configured trap range (512 units) and the AI can afford the cost. Traps do **not** fire during the Reserve phase (the AI is rebuilding its reserve). Before the first reserve is ever met, traps never fire — the AI focuses entirely on spawning zombies first. If a trap fires mid-spawn-wave and consumes resources, the remaining resources are immediately available for spawning — the AI does not pause or wait to re-reserve. Explosive barrels use the same range and cooldown rules as traps.
